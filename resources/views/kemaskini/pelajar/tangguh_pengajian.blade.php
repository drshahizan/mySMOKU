				...
				<div class="flex-lg-row-fluid mb-10 mb-lg-0 me-lg-7 me-xl-10">
					...
					<div class="card">
						...
						<div class="card-body p-12">
							<!--begin::Form-->
							<form action="{{ route('hantar.tangguh.pengajian') }}" method="post" enctype="multipart/form-data">
								@csrf
								<!--begin::Wrapper-->
								...
								<table class="table table-bordered table-striped">
									...
									<tbody>
										{{-- SIJIL TAMAT / SENAT --}}
										<tr>
											<td>Surat Kelulusan Penangguhan Universiti&nbsp;<a href="/assets/contoh/surat_tamat_pengajian__transkrip_akademik.pdf" target="_blank" data-bs-toggle="tooltip" title="contoh"><i class="fa-solid fa-circle-info"></i></a></td>
											<td>
												<input type="file" id="suratTangguh" name="suratTangguh[]" required/>
												@if(session()->has('uploadedsuratTangguh'))
													@foreach(session('uploadedsuratTangguh') as $surat)
														<a href="{{ asset('assets/dokumen/surat_tangguh/' . $surat) }}" target="_blank">{{ $surat }}</a>
													@endforeach
												@endif
											</td>
										</tr>
											...
											<td>
												...
												@if(session()->has('uploadedSokongan'))
													@foreach(session('uploadedSokongan') as $sokongan)
														<a href="{{ asset('assets/dokumen/surat_tangguh/' . $sokongan) }}" target="_blank">{{ $sokongan }}</a>
													@endforeach
												@endif
											</td>
										</tr>
