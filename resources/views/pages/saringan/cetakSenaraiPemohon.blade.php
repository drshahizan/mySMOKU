<!DOCTYPE html>
<html>
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    <title>Cetak Senarai Pemohon</title>
    <link rel="stylesheet" href="assets/css/style.bundle.css">
    <link rel="stylesheet" href="assets/css/saringan.css">
</head>
<style>
    th,td{
        border: 1px solid black!important;
    }
    body{
        font-size: 11px!important;
    }
</style>
<body>
    <div class="header" style="padding-top: 10px; text-align:center; display:block;">
        <div class="image">
            <img src="logoKPT.png" alt="Kementerian Pengajian Tinggi" style=" width:10%; height:10%;">
        </div>
        <br>
        <div class="tittle" style="font-size: 14px;">
            <b>KEMENTERIAN PENGAJIAN TINGGI</b>
            <br>MINISTRY OF HIGHER EDUCATION<br>
        </div>
    </div>

    <hr style="border:solid 1px black; width: 100%;">

    <div style="margin: 10px;display: block;">
        <br>
        <h2 style="text-align: center">SENARAI SARINGAN PERMOHONAN BKOKU</h2>
        <br>
        <table width="100%" style="border: 1px solid black!important;" id="sortTable" class="table table-striped">
            <thead>
                <tr>
                    <th style="width: 15%"><b>ID Permohonan</b></th>
                    <th style="width: 5%"><b>Jenis OKU</b></th>                                        
                    <th style="width: 15%"><b>Nama</b></th>
                    <th style="width: 10%"><b>No. Kad Pengenalan</b></th>
                    <th style="width: 10%"><b>Peringkat</b></th>
                    <th style="width: 15%"><b>Kursus</b></th>
                    <th style="width: 10%"><b>Universiti</b></th>
                    <th style="width: 10%" class="text-center"><b>Tarikh Mula Pengajian</b></th>
                    <th style="width: 10%" class="text-center"><b>Tarikh Tamat Pengajian</b></th>
                </tr>
            </thead>
            <tbody>
                <tr>                                            
                    <td>KPTBKOKUB980112105666</td>
                    <td>BKOKU</td>                                       
                    <td>Aishah Binti Samsudin</td>
                    <td>020112105666</td>
                    <td>Diploma</td>
                    <td>Accountant</td>
                    <td>UMT</td>
                    <td class="text-center">02/07/2020</td>
                    <td class="text-center">20/07/2023</td>
                </tr> 
                <tr>                                            
                    <td>KPTBKOKUM970703041223</td>
                    <td>BKOKU</td>
                    <td>Mohd Ali Bin Abu Kassim</td>
                    <td>970703041223</td>
                    <td>Ijazah</td>
                    <td>Software Engineering</td>
                    <td>UTM(Skudai)</td>
                    <td class="text-center">03/09/2019</td>
                    <td class="text-center">27/07/2023</td>
                </tr> 
                <tr>                                            
                    <td>KPTBKOKUB970204052445</td>
                    <td>BKOKU</td>
                    <td>Sarah Binti Yusri</td>
                    <td>970204052445</td>
                    <td>Ijazah</td>
                    <td>Arts in Mobile Media</td>
                    <td>UITM(Shah Alam)</td>
                    <td class="text-center">15/09/2019</td>
                    <td class="text-center">05/07/2023</td>
                </tr> 
                {{-- <tr>
                    <td>KPTBKOKUM970703041223</td>
                    <td>Santosh A/L Ariyaran</td>
                    <td>BKOKU</td>                                        
                    <td class="text-center">10/7/2023</td>
                    <td class="text-center">Belum Disaring</td>
                </tr> <tr>
                    <td>KPTBKOKUM960909105668</td>
                    <td>Ling Kai Jie</td>
                    <td>BKOKU</td>                                        
                    <td class="text-center">9/7/2023</td>
                    <td class="text-center">Belum Disaring</td>
                </tr> <tr>
                    <td>KPTBKOKUM950804082447</td>
                    <td>Akmal Bin Kairuddin</td>
                    <td>BKOKU</td>                                        
                    <td class="text-center">7/7/2023</td>
                    <td class="text-center">Belum Disaring</td>
                </tr>
                <tr>
                    <td>KPTBKOKUD021212050334</td>
                    <td>Santishwaran A/L Paven</td>
                    <td>BKOKU</td>                                        
                    <td class="text-center">5/7/2023</td>
                    <td class="text-center">Belum Disaring</td>
                </tr>
                <tr>
                    <td>KPTBKOKUP890201065225</td>
                    <td>Ezra Hanisah Binti Md Yunos</td>
                    <td>BKOKU</td>                                    
                    <td class="text-center">9/2/2023</td>
                    <td class="text-center">Belum Disaring</td>
                </tr><tr>
                    <td>KPTBKOKUD010305058473</td>
                    <td>Arshahad Bin Kairul Zaman</td>
                    <td>BKOKU</td>                                        
                    <td class="text-center">7/7/2023</td>
                    <td class="text-center">Belum Disaring</td>
                </tr>
                <tr>
                    <td>KPTBKOKUB981004045253</td>
                    <td>Syed Abdul Kassim Hussain Yusof</td>
                    <td>BKOKU</td>                                        
                    <td class="text-center">5/7/2023</td>
                    <td class="text-center">Belum Disaring</td>
                </tr>
                <tr>
                    <td>KPTBKOKUP940524032341</td>
                    <td>Rahman Mohammed Arshahad Al-dhaqm</td>
                    <td>BKOKU</td>                                    
                    <td class="text-center">9/2/2023</td>
                    <td class="text-center">Belum Disaring</td>
                </tr> --}}
            </tbody>
        </table>
    </div>
</body>
</html>