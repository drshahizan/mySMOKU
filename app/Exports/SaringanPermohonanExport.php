...
class SaringanPermohonanExport implements FromCollection, WithHeadings, WithMapping, WithEvents, ShouldAutoSize
{
    private $programCode;
    private $institusi;
    private $bil = 1;
    ...
    private function tajaan($row)
    {
        if (!empty($row->sumber_lain)) {
            return $this->uppercase($row->sumber_lain);
        }

        if (!empty($row->penaja_lain)) {
            return $this->uppercase($row->penaja_lain);
        }

        return $this->uppercase($row->biaya ?: $row->penaja);
    }
