...
class SaringanTuntutanExport implements FromCollection, WithHeadings, WithMapping, WithEvents, ShouldAutoSize
{
    private $programCode;
    private $institusi;
    private $bil = 1;

    public function __construct($programCode, $institusi = null)
    {
    ...
    {
        $query = DB::table('tuntutan as t')
            ->join('permohonan as a', 'a.id', '=', 't.permohonan_id')
            ->join('smoku as d', 'd.id', '=', 't.smoku_id')
            ->join('smoku_akademik as b', function ($join) {
                $join->on('b.smoku_id', '=', 't.smoku_id')
                    ->where('b.status', 1);
            })
            ->join('bk_info_institusi as c', 'c.id_institusi', '=', 'b.id_institusi')
            ->leftJoin('smoku_butiran_pelajar as bp', 'bp.smoku_id', '=', 't.smoku_id')
            ->leftJoin('smoku_waris as w', 'w.smoku_id', '=', 't.smoku_id')
        ...
        if ($this->programCode === 'ALL') {
            ...
            $query->where('a.program', 'BKOKU')
                ->where('c.jenis_institusi', $this->jenisInstitusi());
        }

        if (!empty($this->institusi)) {
            $query->where('c.nama_institusi', $this->institusi);
        }
    ...
    private function jenisInstitusi()
    {
        $jenis = [
            'UA' => 'UA',
            'POLI' => 'P',
        ...
        return $jenis[$this->programCode] ?? $this->programCode;
    }
