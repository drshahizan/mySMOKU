...
class DrafSPBB1a implements FromCollection, WithHeadings, WithColumnWidths, WithEvents, WithMapping
{
    /**
    ...
    */
    ...
    public function collection()
    {
        ...
        $senarai = Permohonan::join('smoku as b', 'b.id', '=', 'permohonan.smoku_id')
            ...
            ->where('d.jenis', 'Yuran')
            ->whereIn('f.id_institusi', $this->id_institusi)
            ->whereBetween('permohonan.tarikh_hantar', [$tarikhMula, $tarikhTamat])
            ->select(
                'b.id',
                'b.nama',
                'b.no_kp',
    ...
    public function headings(): array
    {
        return [
            ...
            [''],
            ['BORANG PERMOHONAN PERUNTUKAN PROGRAM BKOKU (SPBB 1a)'], 
    ...
    {
        return DB::table('bk_info_institusi')->where('id_institusi', $this->id_user->id_institusi)->value('nama_institusi');
    }

    private function getNamaPenerimaData()
    {
        return DB::table('maklumat_bank')->where('institusi_id', $this->id_user->id_institusi)->value('nama_akaun');
    }

    private function getBankData()
    {
        return DB::table('maklumat_bank')->join('senarai_bank','senarai_bank.kod_bank','=','maklumat_bank.bank_id' )->where('institusi_id', $this->id_user->id_institusi)->value('senarai_bank.nama_bank');
    }

    private function getNoAkaunData()
    {
        return DB::table('maklumat_bank')->where('institusi_id', $this->id_user->id_institusi)->value('no_akaun');
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $startRow = 7; // Set the starting row for borders
