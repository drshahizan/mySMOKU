...
class PenyelarasController extends Controller
{
    ...
    public function permohonanBaharu($id)
    {
        ...
        $akademik = Akademik::orderBy('id', 'desc')
            ->where('smoku_id', $id)
            ->where('status', 1)
            ->first();
        if ($akademik) {
    ...
    {   
        ...
        Validator::make($request->all(), [
            ...
            'akuanPendapatan' => ['nullable', 'file', $extRule],
    ...
    public function senaraiPermohonanBaharu()
    {
        ...
        $smoku = Smoku::leftJoin('permohonan', 'permohonan.smoku_id', '=', 'smoku.id')
            ...
            ->leftJoin('bk_info_institusi', 'bk_info_institusi.id_institusi', '=', 'sa.id_institusi')
            ->where('permohonan.status', '2')
            ->whereIn('sa.id_institusi', $idInstitusiList)
            ->orderBy('permohonan.tarikh_hantar', 'DESC')
            ->get(['smoku.*', 'permohonan.*', 'sa.*', 'bk_info_institusi.nama_institusi']);
    ...
    public function senaraiTuntutanBaharu()
    {
        ...
        $layak = Smoku::join('permohonan', 'permohonan.smoku_id', '=', 'smoku.id')
                ...
                ->leftJoin('bk_peringkat_pengajian as bpp', 'bpp.kod_peringkat', '=', 'sa.peringkat_pengajian')
                ...
                ->select(
                    ...
                    'bpp.peringkat as akademik_peringkat'
                )
                ->distinct()
                ->get();
    ...
    {   
        ...
        $akademik = Akademik::leftJoin('bk_info_institusi', 'bk_info_institusi.id_institusi', '=', 'smoku_akademik.id_institusi')
            ->leftJoin('bk_peringkat_pengajian as bpp', 'bpp.kod_peringkat', '=', 'smoku_akademik.peringkat_pengajian')
            ->where('smoku_id', $id)
            ->where('smoku_akademik.status', 1)
            ->select(
                'smoku_akademik.*',
                'bk_info_institusi.nama_institusi as akademik_nama_institusi',
                'bpp.peringkat as akademik_peringkat'
            )
            ->first();
    ...
    public function hantarTuntutan(Request $request, $id)
    {
        $request->validate([
            ...
            'amaun_wang_saku' => 'required|numeric|min:0',
        ]);
        ...
        // Emel kepada pelajar
        $pelajar = Smoku::find($id);
        $cc_pelajar = $pelajar?->email;

        // Emel kepada penyelaras
        $penyelaras = User::where('no_kp', Auth::user()->no_kp)->first();
        $emel_penyelaras = $penyelaras?->email;

        if ($emel_penyelaras && $cc_pelajar) {
            $catatan = "Tuntutan";
            $emel = EmelKemaskini::where('emel_id', 14)->first();

            Mail::to($emel_penyelaras)
                ->cc($cc_pelajar)
                ->send(new TuntutanHantar($catatan, $emel));
        }
    ...
    public function sejarahTuntutan()
    {
        ...
        $tuntutan = Tuntutan::join('permohonan', 'permohonan.id', '=', 'tuntutan.permohonan_id')
            ...
            ->select(
                ...
                'bk_info_institusi.nama_institusi as akademik_nama_institusi',
                'bpp.peringkat as akademik_peringkat',
                'sa.nama_kursus as akademik_nama_kursus'
            )
            ->orderBy('tuntutan.id', 'DESC')
            ->get();
    ...
    public function sejarahPermohonan()
    {
        ...
        $permohonan = Permohonan::leftJoin('smoku_akademik as sa', function ($join) {
            $join->on('sa.id', '=', DB::raw("(
                SELECT sa2.id
                FROM smoku_akademik sa2
                WHERE sa2.smoku_id = permohonan.smoku_id
                AND sa2.peringkat_pengajian =
                    CAST(NULLIF(SUBSTRING_INDEX(SUBSTRING_INDEX(permohonan.no_rujukan_permohonan,'/',2),'/',-1), '') AS UNSIGNED)
    ...
    {   
        ...
        return view('kemaskini.penyelaras.profil_pelajar_institusi',compact('smoku','butiranPelajar','negeri','keturunan','agama','bandar','parlimen','dun','oku','waris','hubungan','akademik','institusi','peringkat','kursus','mod','biaya','penaja','penajaArray','dokumen','permohonan','bolehKemaskiniProfil','senaraiDokumen'));
    }
